...
class File(models.Model):
	...
	employee_user_id = fields.Many2one('res.users','Employee User',related="employee_id.user_id")
	...
	def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
		...
		if self.env.user.has_group('base.group_system'):
			...
			domain += [['directory_id.employee_id.user_id','=',user_id.id]]
		return super(File, self).search_read(domain, fields, offset, limit, order)
		...
		for vals in vals_list:
			if len(vals_list) > 1:
				if not vals.get('directory_id'):
					name = vals.get('name')
					directory_name = name.split(' ',1)[0]
					directory = self.env['dms.directory'].sudo().search([('name','=',directory_name)],limit=1)
					if directory:
						vals['directory_id'] = directory.id
					else:
						raise ValidationError(_("No directory find for %s") % (name))
