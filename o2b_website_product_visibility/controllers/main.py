...
class ProductVisibilityCon(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        # Filter the category (or categories) based on the computed 'visible' field.
        category = category.filtered(lambda cat: cat.visible)
        ...
        # ---------------------------------------------
        # Filter products to show only those whose
        # public_categ_ids intersect with visible categories.
        # ---------------------------------------------
        visible_category_ids = Category.search([]).filtered(lambda cat: cat.visible).ids
        search_product = search_product.filtered(
            lambda prod: bool(set(prod.public_categ_ids.ids) & set(visible_category_ids))
        )
        ...
        # Get website domain and search categories
        ...
        # Filter categories to include only visible ones.
        categs = categs.filtered(lambda cat: cat.visible)
