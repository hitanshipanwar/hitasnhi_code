...
class ResUsers(models.Model):
    ...
    def _check_password_rules(self, password):
        ...
        if numbers_count < company_id.password_numeric:
            raise ValidationError(self.password_match_message())

        if uppercase_count < company_id.password_upper:
            raise ValidationError(self.password_match_message())

        if lowercase_count < company_id.password_lower:
            raise ValidationError(self.password_match_message())

        if special_count < company_id.password_special:
            raise ValidationError(self.password_match_message())
