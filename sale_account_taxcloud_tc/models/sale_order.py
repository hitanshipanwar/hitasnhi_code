...
class SaleOrder(models.Model):
    ...
    def write(self,vals):
        ...
        for order in self:
            if order.is_taxcloud and order.state == "sale" and "order_line" in vals:
                order.validate_taxes_on_sales_order()
            if order.is_taxcloud and order.state == "sale" and "partner_shipping_id" in vals:
                raise UserError("You can't change the delivery address once the sale order is confirmed when using TaxCloud."
                        "\nIf you still need to change the delivery address, please reset the order to a quotation and then update the delivery address.")
        return res

    @api.onchange('partner_shipping_id')
    def _onchange_warning_partner_shipping_id(self):
        res = {}
        taxcloud_warning = self.is_taxcloud and self.state == "sale" and self.partner_shipping_id and (self._origin.partner_shipping_id.id != self.partner_shipping_id.id)
        if taxcloud_warning:
            res['warning'] = {
                'title': _('TaxCloud Warning!'),
                'message': _(
                    "You can't change the delivery address once the sale order is confirmed when using TaxCloud."
                    "\nIf you still need to change the delivery address, please reset the order to a quotation and then update the delivery address.",
                )
            }
        return res
