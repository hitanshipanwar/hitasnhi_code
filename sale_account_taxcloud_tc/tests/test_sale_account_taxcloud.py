...
from odoo.addons.account_taxcloud_tc.tests.common import TestAccountTaxcloudCommon
...
class TestSaleAccountTaxCloud(TestAccountTaxcloudCommon):
    def test_01_taxcloud_full_flow(self):
        ...
        self.assertEqual(
            downpayment_invoice.invoice_line_ids.tax_ids,
            tax,
            "Tax on the downpayment should be the same than on the SO."
        )
    ...
    def test_compute_taxes_on_send(self):
        ...
        for line in sale_order.order_line:
            self.assertEqual(
                len(line.tax_id), 0, "There should be no tax rate on the line."
            )
