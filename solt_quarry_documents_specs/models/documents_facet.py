...
class TagsCategories(models.Model):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get('documents_specs_folder') and not res.get('folder_id'):
            res['folder_id'] = self.env.context.get('documents_specs_folder')
        return res

    def _get_facet_domain(self, domain):
        if 'documents_specs_folder' not in self.env.context:
            return None
        folder_id = self.env.context.get('documents_specs_folder')
