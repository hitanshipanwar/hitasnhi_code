...
class ImportClient(models.TransientModel):
    ...
    def import_inventory_adjustment_action(self):
        if self.file_option == 'csv':
            ...
            stock_info = self.env['stock.quant']
            ...
            for line in csv_reader:
                ...
                if line.get('Lot Serial Number'):
                    lot_number = lot_number.search([('name', '=', line.get('Lot Serial Number'))], limit=1)
                ...
                if self.import_location == True:
                    res = {
                        ...
                        'inventory_date': date,
                        'lot_id':lot_number.id,
                        'package_id':package_info.id,
                        'quantity': line.get('Quantity'),
                    }

                    info = self.env['stock.quant'].create(res)
