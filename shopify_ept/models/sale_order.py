...
class SaleOrder(models.Model):
    ...
    reconciliation_order = fields.Boolean(string="Reconciliation Order", copy=False)
    is_corrected = fields.Boolean(string="Is Corrected", copy=False)

    def action_reset_order(self):
        for order in self:
            pickings = self.env['stock.picking'].search([('sale_id', '=', order.id)])
            stock_moves = self.env['stock.move'].search([('origin', '=', order.name)])
            # stock_moves = self.env['stock.move'].search([('picking_id', '=', picking.id)])
            _logger.info("stock_moves ===========: %s" % stock_moves)
            for move in stock_moves:
                if move.state != 'cancel':
                    move.write({'state': 'draft'})
                move.unlink()
            for picking in pickings:
                time_trackings = self.env['time.tracking'].search([('picking_id', '=', picking.id)])
                for time_tracking in time_trackings:
                    time_tracking.unlink()
            ...
            for invoice in invoices:
                ...
                invoice.unlink()
            order.write({'state': 'draft', 'is_corrected': True})

        return True

    # ====================O2B=================
    # locked = fields.Boolean(string='Locked', default=False,copy=False)

    # def action_toggle_lock_order(self):
    #     if self.locked:
    #         # Unlock the order
    #         self.with_context(is_unlock=True).write({'locked': False})
    #         # _logger.info('Order %s has been unlocked', self.name)
    #     else:
    #         # Lock the order
    #         self.write({'locked': True})
    #         # _logger.info('Order %s has been locked', self.name)


    # def write(self, vals):
    #     for order in self:
    #         if order.locked and not self.env.context.get('is_unlock'):
    #             raise UserError(_('This order is locked and cannot be modified.'))
    ...
    def import_shopify_orders(self, order_data_lines, log_book):
            ...
            try:
                if sale_order.shopify_order_status == "fulfilled":
                    sale_order.with_context(
                        log_book_id=log_book.id, order_data_line=order_data_line, log_book=log_book).shipped_order_workflow_ept(sale_order)
                ...
                sale_order.reconciliation_order = True
                if order_data_line:
                    order_data_line.write({"state": "failed", "processed_at": datetime.now(),
        ...
        _logger.info("Order ids: %s " % order_ids)
        # ====================O2B==================
        # if order_ids:
        #     order_records = self.env['sale.order'].browse(order_ids)
    ...
    def shipped_order_workflow_ept(self, orders):
        ...
        if not self.shopify_instance_id:
            return super(SaleOrder, self).shipped_order_workflow_ept(orders)
        self.ensure_one()
        stock_location_obj = self.env["stock.location"]
        product_product_obj = self.env["product.product"]
        order_data_line = self._context.get('order_data_line')
        log_book = self._context.get('log_book')
        ...
        shipped_orders.with_context(order_data_line=order_data_line, log_book=log_book).validate_and_paid_invoices_ept(
    ...
    def validate_and_paid_invoices_ept(self, work_flow_process_record):
        ...
        if work_flow_process_record.create_invoice:
            if self.order_line.filtered(lambda line: line.product_id.tracking != "none"):
                self.process_remaining_stock_move(self, order_data_line, log_book)
    ...
    def request_for_shopify_order(self, sale_order):
        ...
        try:
            order = shopify.Order.find(sale_order.shopify_order_id)
            fulfillment_orders = shopify.fulfillment.FulfillmentOrders.find(order_id=int(sale_order.shopify_order_id))
            order_data = order.to_dict()
            if order_data.get('fulfillment_status') == 'fulfilled':
                ...
                pickings  = sale_order.picking_ids.filtered(lambda l: l.state == 'done' and l.updated_in_shopify)
                picking = pickings[-1]
                fulfillment_id = picking.shopify_fulfillment_id
                _logger.info("fulfillment_id %s", fulfillment_id)
                if fulfillment_id:
                    try:
                        fulfillment = shopify.fulfillment.Fulfillment.find(fulfillment_id, order_id=sale_order.shopify_order_id)
                        if fulfillment and fulfillment.id:
                            _logger.info("Found fulfillment %s for order %s", fulfillment.id, sale_order.shopify_order_id)
                            tracking_info = {
                                "number": picking.carrier_tracking_ref or '',
                                "company": picking.carrier_id.shopify_tracking_company or picking.carrier_id.shopify_source \
                                          or picking.carrier_id.name or '',
                                "url": picking.carrier_tracking_url or ''
                            }
                            _logger.info("Updating tracking with info: %s", tracking_info)
                            fulfillment.update_tracking(tracking_info, notify_customer=True)
                            _logger.info("Tracking updated in Shopify for fulfillment ID %s", fulfillment.id)
                        else:
                            _logger.error("Fulfillment object not valid or missing ID.")
                    except Exception as e:
                        _logger.exception("Exception while updating tracking info for fulfillment ID %s: %s", fulfillment_id, str(e))
                else:
                    _logger.warning("No fulfillment ID found for fulfillment_order %s", fulfillment_order.id)
    ...
    def prepare_vals_for_fulfillment(self, sale_order, shopify_location_id, tracking_numbers, picking, carrier_name,
        ...
        if picking.show_mo_validation and picking.carrier_id and picking.carrier_id.delivery_type == 'fixed' and picking.carrier_id.name not in ['Free Shipping','table.181132', 'Intuitive Shipping']:
            template = self.env.ref('shopify_ept.ace_fulfillment_notification_template')
            message = f"Shipping confirmation email sent to {picking.partner_id.email}."
            shopify_order_id = sale_order.shopify_order_id
            shopify_order = shopify.Order().find(shopify_order_id)
            shopify_order.note = (shopify_order.note or '') + "\n" + message
            shopify_order.save()
            if template:
                template.send_mail(picking.id, force_send=True)
        else:
            notify_customer = bool(tracking_numbers and any(tracking_numbers))
