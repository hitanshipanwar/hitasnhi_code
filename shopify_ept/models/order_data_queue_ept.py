...
class ShopifyOrderDataQueueEpt(models.Model):
    ...
    def shopify_create_order_data_queues(self, instance, from_date, to_date, created_by="import",
        ...
        if order_type not in ["shipped", "buy_with_prime"]:
            ...
            order_queues = self.shopify_shipped_order_request(instance, from_date, to_date, created_by="import",
                                                              order_type="shipped")
            instance.last_shipped_order_import_date = to_date - timedelta(hours=1)
        else:
    ...
    def list_all_orders(self, result, instance, created_by, queue_type):
        ...
        while result:
            ...
            if not shopify.ShopifyResource.connection.response:
                link = result.metadata and result.metadata.get('headers', {}).get('Link', {}) if result.metadata.get(
                    'headers', {}).get('Link', {}) else result.metadata.get('headers', {}).get('link', {})
            else:
        ...
        if order_ids:
            ...
            if results:
                ...
                if instance.create_shopify_orders_webhook:
                    order_queues = self.create_order_queue_for_webhook_order(results, instance, queue_type,
                                                                             created_by="import")
                else:
                    ...
                    # ======================O2B====================
                    # if order_queues:
                    #     order_queue_ids = [queue.id for queue in order_queue_obj.browse(order_queues)]
                    #     self.process_order_queue_data(order_queue_ids)
                    # ======================O2B====================
    ...
    # ======================O2B====================
    # def process_order_queue_data(self, order_queue_ids):
    #     """
    #     Processes order data from the given queue IDs.
    #     :param order_queue_ids: List of order queue IDs to process.
    #     """
    #     order_queue_obj = self.env["shopify.order.data.queue.ept"]
    #     order_data_queue_line_obj = self.env["shopify.order.data.queue.line.ept"]
    #     sale_order_obj = self.env['sale.order']
    #     # _logger.info("process_order_queue_data Calling nad Queue Id: %s " % order_queue_ids)
    #     for queue_id in order_queue_ids:
    #         queue = order_queue_obj.browse(queue_id)
    #         if queue:
    #             # _logger.info("Processing Queue ID %s with lines: %s", queue_id, queue.order_data_queue_line_ids)
    #             for line in queue.order_data_queue_line_ids:
    #                 # _logger.info("Processing Order Line ID %s with data: %s", line.id, line.order_data)
                    
    #                 # Ensure order_data is a dictionary
    #                 try:
    #                     order_data = json.loads(line.order_data) if isinstance(line.order_data, str) else line.order_data
    #                 except json.JSONDecodeError:
    #                     # _logger.info("Error decoding order data for line ID %s: %s", line.id, line.order_data)
    #                     continue

    #                 if not isinstance(order_data, dict):
    #                     # _logger.info("Order data for line ID %s is not a dictionary: %s", line.id, order_data)
    #                     continue
                    
    #                 # Check if the order already exists in Odoo
    #                 # _logger.info("Existing Order- Order Data: %s " % order_data)
    #                 # _logger.info("Existing Order- Order Data: %s " % order_data.get('name'))
    #                 existing_order = sale_order_obj.search([('name', '=', order_data.get('name')), ('company_id', '=', queue.shopify_instance_id.shopify_company_id.id)])
    #                 # _logger.info("Existing Order %s", existing_order)
    #                 if existing_order:
    #                     # Update the existing order
    #                     existing_order = existing_order[0]
    #                     # _logger.info("Found existing order %s", existing_order.name)
                        
    #                     # Extract existing product SKUs from the order lines
    #                     existing_product_skus = existing_order.order_line.mapped('product_id.default_code')
                        
    #                     # Filter new items that are not in the existing order
    #                     shopify_items = order_data.get('line_items', [])
    #                     # _logger.info("New Items %s", shopify_items)
                        
    #                     unique_items_by_sku = {}
    #                     for item in shopify_items:
    #                         sku = item.get('sku')
    #                         unique_items_by_sku[sku] = item

    #                     unique_items = list(unique_items_by_sku.values())
    #                     # _logger.info("Unique shopify Items %s", unique_items)
    #                     if unique_items:
    #                         for item in unique_items:
    #                             # product_sku = item.get('sku')  # Adjust this key based on your actual data structure
    #                             product_variant_id = item.get('variant_id')  # Adjust this key based on your actual data structure
    #                             print("Searching for product with SKU:", product_variant_id)

    #                             # Search for product by SKU
    #                             product = self.env['shopify.product.product.ept'].search([('variant_id', '=', product_variant_id)], limit=1)
    #                             # product = self.env['product.product'].search([('default_code', '=', product_sku)], limit=1)
    #                             if product:
    #                                 # Check if the product line already exists
    #                                 existing_line = existing_order.order_line.filtered(lambda l: l.product_id and l.product_id.id == product.product_id.id)
    #                                 if existing_line:
    #                                     existing_line = existing_line[0]
    #                                 if not existing_line:
    #                                     existing_order.order_line.create({
    #                                         'shopify_line_id': item.get('id'),
    #                                         'order_id': existing_order.id,
    #                                         'company_id': existing_order.company_id.id,
    #                                         'product_id': product.product_id.id,
    #                                         'product_uom_qty': item.get('current_quantity'),
    #                                         'price_unit': item.get('price'),
    #                                         'name': product.name
    #                                     })
    #                                     # _logger.info("Added new order line for product %s", product.name)
    #                                 else:
    #                                     update_values = {}
    #                                     existing_line_id = int(existing_line.shopify_line_id)
    #                                     incoming_id = int(item.get('id'))
    #                                     if existing_line_id != incoming_id:
    #                                         update_values['shopify_line_id'] = item.get('id')
    #                                     new_quantity = item.get('current_quantity')
    #                                     delivered_quantity = max(existing_line.mapped('qty_delivered'))
    #                                     if existing_line.product_uom_qty != item.get('current_quantity'):
    #                                         if delivered_quantity <= new_quantity:
    #                                             update_values['product_uom_qty'] = new_quantity
    #                                     if existing_line.price_unit != round(float(item.get('price')), 1):
    #                                         update_values['price_unit'] = item.get('price')
    #                                     if existing_line.name == product.name:
    #                                         update_values['name'] = product.name
    #                                     if update_values:
    #                                         existing_line.write(update_values)
    #                                     # _logger.info("Order line for product %s already exists", product.name)
    #                             else:
    #                                 _logger.warning("Product not found in Odoo")
    #                     else:
    #                         _logger.info("All items are already present in the order")
    #                 else:
    #                     _logger.warning("Order %s does not exist in Odoo", order_data.get('name'))

    #     return True
    # ======================O2B====================

    def create_order_queue_for_webhook_order(self, results, instance, queue_type, created_by="import"):
        """
        This method is used to create order queue for webhook for import specific order.
        @author: Yagnik Joshi @Emipro Technologies Pvt. Ltd on date 05 December 2023 .
        """
        sale_order_obj = self.env['sale.order']
        order_data_queue_line_obj = self.env["shopify.order.data.queue.line.ept"]
        existing_shopify_orders = self.search_existing_shopify_orders(results, instance)
        order_list = []
        new_order_list = []
        order_queues = []
        for result in results:
            if str(result.id) in existing_shopify_orders:
                order_list.append(result)
            else:
                new_order_list.append(result)
        if order_list:
            for orders in order_list:
                order_data = orders.to_dict()
                sale_order_obj.sudo().process_shopify_order_via_webhook(order_data, instance, True)
        if new_order_list:
            order_queues += order_data_queue_line_obj.create_order_data_queue_line(new_order_list, instance, queue_type,
                                                                                   created_by="import")
        return order_queues

    def search_existing_shopify_orders(self, results, instance):
        """
        This method will find existing shopify orders.
        """
        order_list = [str(result.id) for result in results]
        if len(order_list) > 1:
            domain_for_orders = "in %s" % str(tuple(order_list))
        else:
            domain_for_orders = "= '%s'" % (tuple(order_list)[0])
        existing_orders_qry = """select shopify_order_id from sale_order where shopify_instance_id = %s and shopify_order_id %s""" % (
            instance.id, domain_for_orders)
        self._cr.execute(existing_orders_qry)
        existing_order_data = self._cr.dictfetchall()
        existing_order = [queue_data.get('shopify_order_id') for queue_data in existing_order_data]
