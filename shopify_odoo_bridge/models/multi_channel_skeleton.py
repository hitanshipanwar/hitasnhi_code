		...
		if channel_id.channel == 'shopify':
			...
			if order_id and order_id.order_line:
				...
				if order_state in ['cancelled']:
					...
					if create_invoice:
						validated_delivery = order_id.picking_ids.filtered(lambda p: p.state == 'done')
						if not validated_delivery:
							create_invoice = False
							invoice_state = False
							status_message += f"Delivery Order not validated for order {order_id.name}. Invoice not created.<br/>"
