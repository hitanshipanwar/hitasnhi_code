...
class Bridge:
	...
	def ensure_response(self,obj,method,*args,**kwargs):
		if method in ['save','destroy']:
			...
			try:
				...
				_logger.info(f"==================={obj}_id = {args} data not found ===========")
				return False
	...
	def process_order(self,order):
			...
			order_data.update(
				{
					...
					'customer_company' : order.billing_address and order.billing_address.company or False
				}
			)
		...
		for line in order.line_items:
			tax_lines = [t.to_dict() for t in getattr(line, 'tax_lines', [])] #Abhishek
			order_line_data = {
				...
				'line_taxes': [] if all(t.get('price') in ["0", "0.00", 0, 0.0] for t in tax_lines) else self.process_tax(line, order.taxes_included),  #Abhishek
			}
	...
	# Abhishek ---- stop due to fixed price won't come like if rate is 0 and still there is some price it won't show.
		...
		for tax in order_line.tax_lines:
			...
			if rate == 0.0 and price > 0.0:
				taxes.append({
					'included_in_price': taxes_included,
					'name'             : tax.title,
					'rate'             : price,  # Fixed amount
					'tax_type'         : 'fixed'  # Clearly mark as fixed
				})
			else:
				taxes.append({
					'included_in_price': taxes_included,
