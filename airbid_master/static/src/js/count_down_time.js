odoo.define('airbid_master.display_timer_widget_project', function (require) {
'use strict';
...
publicWidget.registry.displayTimerWidget = publicWidget.Widget.extend({
    ...
    _updateCountdown: function (remainingTime) {
        // console.log(this.$el.parent().parent().data('proid'),this.$el.parent().parent().data('status'),"DDDDDDDDDDDDDDDDDDDDDDd",remainingTime)
        var remainingSeconds = remainingTime;

        if(remainingSeconds == 0){
            var proid=this.$el.parent().parent().data('proid')
            var state=this.$el.parent().parent().data('status')
            if(proid && state == 'open'){
                ajax.jsonRpc('/timer/out/state','call',{
                  proid : proid,
        ...
        // var remainingSeconds = remainingSeconds % 86400;
