    ...
    def action_put_in_pack(self):
        ...
        if packaging_id and (packaging_id.package_carrier_type not in ['none']):
            if packaging_id and (packaging_id.max_weight < self.shipping_weight):
                msz = _('Shipment weight should be less then {max_weight} {weight_uom_name} as {max_weight} {weight_uom_name} is the max weight limit set for {name}.'.format(
                    max_weight=packaging_id.max_weight, weight_uom_name=weight_uom_name, name=packaging_id.name))
