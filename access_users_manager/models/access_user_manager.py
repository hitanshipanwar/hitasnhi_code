...
class accessAccessManagement(models.Model):
    ...
    access_user_ids = fields.Many2many('res.users', 'user_management_users_rel', 'user_management_id', 'user_id',
                                   'Users')
    access_user_rel_ids = fields.Many2many('res.users', 'res_user_store_rel', string='Store user profiles', compute='access_compute_profile_ids')
    ...
    @api.onchange('is_profile', 'access_profile_ids', 'access_profile_domain_model')
    def onchange_is_profile(self):
        ...
        for profile in self.access_profile_ids:
            access_rights += profile.implied_ids.model_access.ids
            record_rules += profile.implied_ids.rule_groups.ids
            model_ids.extend(profile.implied_ids.model_access.mapped('model_id').ids)
    ...
    @api.depends('access_profile_ids', 'access_profile_ids.access_user_ids', 'access_profile_ids.implied_ids')
