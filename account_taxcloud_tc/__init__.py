...
def pre_init_hook(env):
    ...
    installed_taxcloud_modules = module_obj.search([('name','in',odoo_taxcloud_modules),('state','=','installed')]).mapped('name')
    ...
    if unavailable_tc_modules:
        raise UserError(_('''Odoo TaxCloud modules\n\n%s\nare in installed state. \n\nBut corresponding TaxCloud Official modules\n\n%s\
                          \nare not available in your system. \n\nPlease download from Odoo Apps Store to proceed with the installation''')
                          % ("\n".join([installed_taxcloud_module for installed_taxcloud_module in installed_taxcloud_modules]),
                             "\n".join([unavailable_tc_module for unavailable_tc_module in unavailable_tc_modules])))