...
class HrPayslip(models.Model):
    ...
    total_working_day = fields.Integer(string="No.Of Working Days")
    emp_worked_day = fields.Integer(string="Employee Time-Off")
    leaves_deduct_amt = fields.Integer(string="Leave Deduction Amount")
    ...
    tds_multi_attachments = fields.Many2many('ir.attachment','ir_attachment_pay_ref','payslip_id','attachment_id', string="TDS Attchment")
    ...
    contract_id = fields.Many2one('hr.contract', string='Contract', required=True, readonly=True, help="Contract",
                                  states={'draft': [('readonly', False)]})
    salary_amount = fields.Float(string="Salary Amount",compute="_compute_salary_amount")
    ...
    def _compute_salary_amount(self):
        for rec in self:
            rec.salary_amount = rec.contract_id.wage if rec.contract_id else 0.0
    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type == 'tree':
            ...
            payslip_user = self.env.user.has_group('hr_payroll_community.group_hr_payroll_community_user_gruops')
            if payslip_user:
        ...
        if view_type == 'form':
            ...
            if payslip_user:
                for node in doc.xpath("//form"):
                    ...
                    node.set('copy','0')
                    node.set('delete','0')
                for node in doc.xpath("//button[@name='action_payslip_done']"):
                    node.set('class','d-none')
                for node in doc.xpath("//button[@name='compute_sheet']"):
                    node.set('class','d-none')
                for node in doc.xpath("//button[@name='action_payslip_cancel']"):
                    node.set('class','d-none')
                res['arch'] = etree.tostring(doc)
        return res
