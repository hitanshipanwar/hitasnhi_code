...
class LLMThread(models.Model):
    ...
    #     for vals in vals_list:
    #         if not vals.get("name"):
    #             model_name = vals.get('model') or 'Unknown'
    #             vals["name"] = f"Chat with {model_name}"

    #     res = super().create(vals_list)
    #     _logger.info("==res : %s" % res)

    #     def trigger_llm_stream(thread_id, message, base_url, session_id):
    #         _logger.info("trigger_llm_stream calling")
    #         url = f"{base_url}/llm/thread/generate"
    ...
    #         _logger.info("---- LLM Stream Response ----")
    #         _logger.info("===response.status_code: %s " % response.status_code)
    #         _logger.info("===response.text: %s" % response.text)
            
    #     for record_instance, vals in zip(res, vals_list):
    #         # record_instance.message_post(body="Hii....... O2b helpdesk ticket created")
    #         _logger.info('==record_instance=: %s' % record_instance)
    #         _logger.info('==vals %s' % vals)
    #         model = vals.get('model')
    #         res_id = vals.get('res_id')
    #         if model and res_id:
    #             target_model = self.env[model].browse(res_id)
    #             _logger.info("===record : %s"  % target_model)

    #             if hasattr(target_model, 'description'):
    #                 # res.message_post(body=target_model.description)
    #                 print("")
    #                 #  Trigger LLM stream
    #                 thread_id = res.id
    #                 message = target_model.description
    #                 base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
    #                 _logger.info("== %s" % base_url)
    #                 # threading.Thread(
    #                 #   target=trigger_llm_stream, args=(thread_id, message, base_url)
    #                 # ).start()
    #                 session_id = request.session.sid
    #                 _logger.info("===session_id=: %s" % session_id)
    #                 threading.Thread(
    #                     target=trigger_llm_stream, args=(thread_id, message, base_url, session_id)
    #                 ).start()
    #                 _logger.info("==done=")
