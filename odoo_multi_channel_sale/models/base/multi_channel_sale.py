...
class MultiChannelSale(models.Model):
    ...
    default_store_id = fields.Many2one(
        ...
        string='Parent Store',
        help=_("""***While using multi store***\n
    ...
    use_miss_product = fields.Boolean(string='Use Missing Product', default=True)
    miss_product = fields.Selection([
        ('missing', 'Use Default Product'),
        ('line product', 'Create New Product'),      
    ], default='missing')

    miss_product_id = fields.Many2one(
        comodel_name='product.product',
        string='Missing Product',
        domain=[('type', '=', 'service')],
        help="""Missing product used in sale order line.""",
    )
    ...
    import_order_date = fields.Datetime(
        ...
        help=_("""A date used for selecting orders created after (or at) a specified time.""")
    )

    update_order_date = fields.Datetime(
        string='Order Updated',
        help=_("""A date used for selecting orders that were last updated after (or at) a specified time.
    ...
    def shopify_order_webhook_data_update(self, event_data, headers, type, **kwargs):
        """
        Parse Shopify order webhook data and return formatted data for Odoo processing.
        """
        ...
        try:
            ...
            _logger.error("[Shopify Webhook] JSON Decode Error: %s", e)
            return {}
        store_id = str(order_data.get('id'))
        order_status = order_data.get('fulfillment_status')
        payment_method = ', '.join(order_data.get('payment_gateway_names', [])) or 'manual'
        financial_status = order_data.get('financial_status') #Abhishek

        update_case = 'status_update'
        ...
        return {
            ...
            "data": [{
                ...
                "financial_status": financial_status, #Abhishek
                "raw_data": order_data, #Abhishek here we can send the raw data so that we get everything in the update order method.
            }]
        }