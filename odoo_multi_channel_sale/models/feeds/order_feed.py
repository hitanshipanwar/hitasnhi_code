...
# & remove this code
        ...
        if line_ids:
            ...
            for line_id in order_line_feed:
                ...
                if line_id.line_source == 'delivery' and type(carrier_id) is not str:
                    ...
                    if not (self.channel_id.use_miss_product or line_id.line_product_id) and product_res.get('message'):
                ...
                if product_id:
                    ...
                    # ADD TAX
                elif self.channel_id.use_miss_product and self.channel_id.miss_product_id and not line_id.line_product_id:
                    prod_id = self.channel_id.miss_product_id
                    if  self.channel_id.miss_product == 'line product':
                         copied_data = prod_id.copy_data()
                         copied_data[0]['name'] = line_id.line_name
                         prod_id = self.env['product.product'].create(copied_data[0])
                         prod_id.product_tmpl_id.write({'active':False})
                    line = dict(
                        name=prod_id.display_name,
                        price_unit=line_price_unit,
    ...
    def _get_tax_name(self, string):
        is_exist = self.env['account.tax'].search([('name', '=', string)])
        if is_exist:
            for i in range(99):
                is_exist = self.env['account.tax'].search(
                    [('name', '=', string + str(i))])
                if not is_exist:
                    return string + str(i)
        return string
    ...
    def get_taxes_ids(self, taxes):
        ...
        for tax in eval(taxes):
            if isinstance(tax, (tuple, list)) and len(tax) == 3 and tax[0] == 6:
                return tax
            ...
            if tax.get('name'):
                domain.append(("tax_name.name", "=", tax.get('name', False)))
            tx_inclusive = None
            ...
            if mapping:
                ...
                tx_name = self._get_tax_name(tx_name)
